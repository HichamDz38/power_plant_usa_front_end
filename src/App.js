...
var f_url = `https://powerplantusasqlite.herokuapp.com/energies/0/`;
var center = [50.8, -90]; // usa coordinate
var zoom =4;  // in order to display all the coutry
if(city && year && limit){
  f_url = `https://powerplantusasqlite.herokuapp.com/energies/${year}/${city}/${limit}/`;
  center = states[cities_map[city]]
  zoom = 6; // zoom allow to show a city
}else if(city && year){
...
if(year){
  var summed_energies={}; // here we get the total generated energies in each state
  const summed_result_url = `https://powerplantusasqlite.herokuapp.com/energies_summary/${year}/`
  axios.get(summed_result_url).then(res => {
    var Data = res.data.results;
    Data.map(d=>(
      summed_energies[d['plant_information__plant__state']]=d['total_annual_net']
    ))
    ...
    useEffect(() => {
      axios.get(url).then(res => {
        ...
        Data.map(d =>(
          ...
          federal state % :${(d.generator_anual_net/summed_energies[d.plant_information.plant.state]*100).toFixed(2)} %<br/>
          `)
          .addTo(map)
